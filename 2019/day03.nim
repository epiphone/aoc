...
proc step1(): int =
  ...
  for wire in input:
    ...
    for step in wire.split(','):
      let
        direction = parse_direction(step[0])
        length = parseInt(step[1..^1])

      for _ in 0..<length:
        let new_loc = (x: curr_loc.x + direction.x, y: curr_loc.y + direction.y)
        wire_points.incl(new_loc)
        curr_loc = new_loc

    wires.add(wire_points)

  var intersections: seq[Point] = @[]
  for point in wires[0]:
    if point != (0, 0) and wires[1].contains(point):
      intersections.add(point)

  var smallest = 9999
  for intersection in intersections:
    let dist = manhattan_distance((0, 0), intersection)
    if dist < smallest:
      smallest = dist
...
proc step2(): int =
  ...
  for wire_index, wire in input.pairs:
    var
      curr_loc = (x: 0, y: 0)
      row_points: seq[tuple[x, y, steps: int]] = @[]
      steps = 1
    ...
    for step in wire.split(','):
      ...
      for _ in 0..<length:
        let new_loc = (x: curr_loc.x + direction.x, y: curr_loc.y + direction.y, steps: steps)
        row_points.add(new_loc)
