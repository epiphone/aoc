...
proc step2(): int =
  ...
  let map = computer.process().mapIt(it.char).join().splitLines()[0..^4]

  var
    pos = (map[^1].find('^'), map.high)
    dir: Dir = Up
    turned_to: Dir = Up
    paths: seq[Path]
    path_len = 0

  func is_scaffolding(point: Point): bool =
    point[0] >= 0 and point[0] < map[0].len and point[1] >= 0 and point[1] < map.len and map[point[1]][point[0]] == '#'

  while true:
    let
      forward = move(pos, dir)
      right = move(pos, turn(dir, Right))
      left = move(pos, turn(dir, Left))

    if is_scaffolding(forward):
      pos = forward
      path_len += 1
    else:
      if path_len > 0:
        paths.add((turned_to, path_len))
        path_len = 0

      if is_scaffolding(right):
        dir = turn(dir, Right)
        turned_to = Right
      elif is_scaffolding(left):
        dir = turn(dir, Left)
        turned_to = Left
      else:
        break

  proc find_subpaths(path_left: seq[Path], subpaths: seq[seq[Path]], path_taken: seq[seq[Path]]): seq[seq[Path]] =
    if subpaths.len > 3:
      return @[]
    if path_left.len == 0:
      return (if subpaths.len == 3: path_taken else: @[])

    for subpath in subpaths:
      if subpath.len <= path_left.len and path_left[0..<subpath.len] == subpath:
        let res = find_subpaths(path_left[subpath.len..^1], subpaths, path_taken & @[subpath])
        if res.len > 0:
          return res

    for end_index in 1..min(5, path_left.high):
      let
        new_subpath: seq[Path] = path_left[0..<end_index]
        new_path_left: seq[Path] = path_left[end_index..^1]
        res = find_subpaths(new_path_left, subpaths & @[new_subpath], path_taken & @[new_subpath])
      if res.len > 0:
        return res

  let
    steps = find_subpaths(paths, @[], @[])
    subpaths = steps.deduplicate()

  computer.inputs = steps.mapIt((65 + subpaths.find(it)).char).join(",").as_ascii()
  discard computer.process()

  for subpath in subpaths:
    computer.inputs = subpath.mapIt(&"{($it[0])[0]},{it[1]}").join(",").as_ascii()
    discard computer.process()

  computer.inputs = "n".as_ascii()
  return computer.process()[^1]
